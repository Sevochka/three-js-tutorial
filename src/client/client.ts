...
const torusKnotPoints = (<THREE.Geometry>torusKnotMesh.geometry).vertices.map(function (v) {
    return new CANNON.Vec3(v.x, v.y, v.z)
})
const torusKnotFaces = (<THREE.Geometry>torusKnotMesh.geometry).faces.map(function (f) {
    return [f.a, f.b, f.c]
})
// @ts-ignore
